<template>
    <div>
        ...
        <!-- 顶栏前景色：Vuetify 的 dark/light 是三态（dark=true 强制深色 → 白字；light=true 强制浅色 →
             深字；两者都不传则继承 v-app 的根主题）。只传 :dark="navIsDark" 是不够的 —— 当品牌色偏亮、
             navIsDark 为 false 而在深色主题下时，dark 既不是 true 也没有 light 兜底，会回落到根主题的
             theme--dark，白字落在浅色品牌色上。所以浅色品牌色必须显式 light=true（见 utils/appearance.js
             的 isLightBackground 与 navForeground）。 -->
        <v-app-bar v-if="$store.state.nav" class="px-0" :class="navIsDark ? 'app-bar--on-dark' : 'app-bar--on-light'" :color="navBgColor" :dark="navIsDark" :light="!navIsDark" dense app fixed clipped-left extension-height="64">
        ...
        <AppDialog
            ...
            :color="appBarColor"
            :light-header="dialogHeaderLight"
            max-width="420"
            :persistent="false"
            dismiss-icon
        >
        ...
        <v-dialog v-model="ai_enabled" persistent max-width="700" scrollable>
            <v-card class="dialog-border d-flex flex-column" style="height: 600px;">
                <v-card-title class="primary white--text py-3 ai-dialog-title">
                    ...
                    <v-spacer></v-spacer>
                    <v-btn icon :dark="navIsDark" :light="!navIsDark" @click="closeAi">
...
export default {
    ...
    computed: {
        ...
        /**
         * AppDialog 外壳 toolbar 是否需要「浅色底 / 深色字」。
         * appBarColor 在上面的分支里可能是用户自定义的浅色品牌色（浅色主题 + 浅品牌色），
         * 而 AppDialog 默认按深色底白字渲染，所以这里要把亮度结论透传给对话框，
         * 否则白字落在浅底上（与顶栏 :light="!navIsDark" 是同一件事）。
         */
        dialogHeaderLight() {
        ...
        items: function () {
            ...
            const userLinks = [
                {
                    ...
                    groups: [
                        { icon: "mdi-account-cog", href: "/user/usersettings", text: "appHeader.user_setting", color: "blue darken-1" },
            ...
            const adminLinks = [
                {
                    ...
                    color: "deep-orange",
                    groups: [
                        { icon: "mdi-cog", href: "/admin/settings", text: "appHeader.systemSettings", color: "blue darken-1"},
                        ...
                        { icon: "mdi-library-shelves", href: "/admin/books", text: "appHeader.bookManagement", color: "blue darken-2"},
            ...
            const navLinks = [
                ...
                { icon: "mdi-folder-multiple-outline", href: "/folder", text: "appHeader.folders", count: this.sys.folders, color: "amber darken-2" },
    ...
    mounted() {
        ...
        this.$backend("/user/info").then((rsp) => {
            ...
            if (process.client && rsp.sys.theme !== '') {
                ...
                // 站点默认总是记录进 store：「重置为默认」要用同一份站点默认去重算 darkMode，
                // 否则会把用户钉在内置默认（深色）上。
                this.$store.commit('appearance/setSiteTheme', rsp.sys.theme);
                const userAppearanceSettings = localStorage.getItem('appearance_settings');
                if (!userAppearanceSettings) {
                    // 站点默认深浅色必须写进 store，不能只改 $vuetify.theme.dark：
                    // store 才是深浅色的运行时真值，只改 Vuetify 会让两者一直不一致 ——
                    // 面板选中态错位，且用户之后动任何一项外观都会用 store 里的旧值把整站主题盖回去。
                    // adoptSiteTheme 不写 localStorage 缓存，站点默认每次跟随管理员设置。
                    this.$store.commit('appearance/adoptSiteTheme', rsp.sys.theme);
                }
            }
            if (process.client) {
