本文主要面向开发者，希望搭建mybooks开发环境，修改代码；或者希望手动搭建环境，满足网站定制化需求的人士。
...
可行的方案如下：
1. 配置一个NGINX服务，将 后台的URL 转发到后台server去；（例如你搭建好的mybooks程序；或者其他可用的后端）
```
server {
    listen 80;
    server_name mydev.com;
    index index.php index.html index.htm;
    root        /data/code/mybooks/app/dist;
...
2. 直接修改 `mybooks.js` 文件中的 `backend()` 函数，把url地址hardcode改掉。（不太建议使用这种方案；会遇到CORS相关的问题，需要有更高的配置技巧）
...
services:
  ...
  dev:
    ...
    volumes:
      ...
      - /data/code/mybooks/webserver:/var/www/mybooks/webserver # 这个是代码
    ports:
      - "8082:80"  # 这个是端口
    environment:
      - TZ=Asia/Shanghai
  ...
  douban-rs-api:
    image: poxenstudio/douban-api-rs
```

然后，就可以尽情修改代码啦！修改完毕后，可以从 `/data/dev/log/mybooks.log` 中查看日志。如果因为语法异常导致程序挂掉，那么可以用`docker restart`来重启下。
...
其次，想要完整手动搭建mybooks程序的话，推荐是在Linux环境下进行搭建；在Windows和Mac环境下搭建的话，我自己没有研究过，可以自行折腾。

### 总体说明

随着版本更新，本文档列举的命令可能不能及时更新；如果遇到问题，建议参考下 [mybooks/calibre-docker项目中的Dockerfile](https://github.com/mybooks/calibre-docker/blob/master/Dockerfile) ，搭建出基础的calibre环境；然后再根据本仓库的[Dockerfile](../Dockerfile)中的指令，安装nodejs和python的依赖。
...
```
cd /var/www/
git clone https://github.com/PoxenStudio/mybooks.git
cd mybooks
```

### 安装calibre基础环境
建议参考：[mybooks/calibre-docker项目中的Dockerfile](https://github.com/mybooks/calibre-docker/blob/master/Dockerfile)
...
# 安装依赖库
pip3 install -r /var/www/mybooks/requirements.txt
...
```
### 安装其他工具（可选）
建议参考：[本仓库的Dockerfile](https://github.com/PoxenStudio/mybooks/blob/master/Dockerfile)
...
cd /var/www/mybooks/app/
npm install
npm build-spa
```
前端，搞定！打包出来的web文件都在 `/var/www/mybooks/app/dist/` 目录中，后续会在nginx中配置使用。

### 创建书库
使用以下命令，基于代码自带的书籍，创建书库：
```
calibredb add --library-path=/data/books/library/  -r /var/www/mybooks/docker/book/
```

或者可以从github下载mybooks.org的书库（非常非常大，极其慢，需要个把小时）
```
git clone https://github.com/mybooks/mybooks-library.git /data/books/library
```

### 创建DB
执行以下命令，创建程序DB。
```
python /var/www/mybooks/server.py --syncdb
